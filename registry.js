...
// Load/save purchase status from localStorage
function loadPurchaseStatus() {
    const saved = localStorage.getItem('registryPurchases');
    return saved ? JSON.parse(saved) : {};
}

function savePurchaseStatus(status) {
    localStorage.setItem('registryPurchases', JSON.stringify(status));
}

function togglePurchaseStatus(index) {
    const status = loadPurchaseStatus();
    status[index] = !status[index];
    savePurchaseStatus(status);
    renderProducts();
}
...
function createModal() {
    ...
    modal.style.cssText = 'display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.6);z-index:1000;align-items:center;justify-content:center;';
    
    const content = document.createElement('div');
    content.style.cssText = 'background:#fff;padding:2.5rem;border-radius:12px;max-width:500px;width:90%;box-shadow:0 10px 40px rgba(0,0,0,0.2);text-align:center;';
    
    const h3 = document.createElement('h3');
    h3.textContent = 'Product Already Purchased';
    h3.style.cssText = 'margin-bottom:1rem;color:#333;font-size:1.3rem;';
    
    const p = document.createElement('p');
    p.textContent = 'Product already purchased. Are you sure you want to make the product purchasable again?';
    p.style.cssText = 'margin-bottom:2rem;color:#666;line-height:1.6;';
    
    const buttons = document.createElement('div');
    buttons.style.cssText = 'display:flex;gap:1rem;justify-content:center;';
    
    const yesBtn = document.createElement('button');
    yesBtn.textContent = 'Yes';
    yesBtn.style.cssText = 'padding:0.75rem 2rem;border:none;border-radius:6px;font-size:1rem;font-weight:600;cursor:pointer;background:#dc2626;color:#fff;';
    yesBtn.onmouseover = () => yesBtn.style.background = '#b91c1c';
    yesBtn.onmouseout = () => yesBtn.style.background = '#dc2626';
    
    const noBtn = document.createElement('button');
    noBtn.textContent = 'No';
    noBtn.style.cssText = 'padding:0.75rem 2rem;border:none;border-radius:6px;font-size:1rem;font-weight:600;cursor:pointer;background:#e5e7eb;color:#333;';
    noBtn.onmouseover = () => noBtn.style.background = '#d1d5db';
    noBtn.onmouseout = () => noBtn.style.background = '#e5e7eb';
    ...
    return { modal, yesBtn, noBtn };
}

const { modal, yesBtn, noBtn } = createModal();
...
function renderProducts() {
    ...
    productsData.forEach((product, index) => {
        const isPurchased = purchaseStatus[index] || false;
        
        const figure = document.createElement('figure');
        figure.className = 'gallery-grid-item';
        if (isPurchased) {
            figure.style.opacity = '0.7';
        }
        ...
        link.href = isPurchased ? '#' : product.link;
        link.className = 'gallery-grid-image-link';
        link.target = '_blank';
        link.rel = 'noopener';
        if (isPurchased) {
        ...
        img.onerror = function() {
            this.outerHTML = '<div style="width:100%;height:100%;background:#ffebee;display:flex;align-items:center;justify-content:center;flex-direction:column;color:#c62828;font-size:0.8rem;padding:10px;text-align:center;">❌ Image failed<br><small>' + product.image + '</small></div>';
        };

        // Red overlay for purchased items (created with JavaScript)
        if (isPurchased) {
            const overlay = document.createElement('div');
            overlay.style.cssText = 'position:absolute;top:0;left:0;width:100%;height:100%;background:rgba(220,38,38,0.85);display:none;align-items:center;justify-content:center;z-index:10;border-radius:8px;';
            
            const overlayText = document.createElement('div');
            overlayText.textContent = 'Purchased';
            overlayText.style.cssText = 'color:#fff;font-weight:600;font-size:1.1rem;';
            
            const bar = document.createElement('div');
            bar.style.cssText = 'position:absolute;width:80%;height:4px;background:#fff;border-radius:2px;';
            
            overlay.appendChild(overlayText);
            overlay.appendChild(bar);
            
            wrapper.onmouseenter = () => overlay.style.display = 'flex';
            wrapper.onmouseleave = () => overlay.style.display = 'none';
